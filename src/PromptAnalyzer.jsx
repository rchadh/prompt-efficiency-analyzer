...
const LLM_FACTORS = [
  ...
  { dim: "Instruction Style", claude: "XML tags excel", gpt4o: "Markdown headers excel", gemini: "Natural language preferred", llama: "Simpler = more reliable", impact: "Formatting of instructions shifts score" },
  { dim: "Role Prompting", claude: "Moderate — resists personas", gpt4o: "High — roles boost output", gemini: "Moderate effect", llama: "High for fine-tuned", impact: "'You are an expert' has variable ROI" },
  { dim: "Chain of Thought", claude: "Excellent (extended thinking)", gpt4o: "Strong with o1/o3", gemini: "Good", llama: "Variable by version", impact: "'Think step by step' ROI differs per model" },
  { dim: "Output Format", claude: "Excellent JSON/XML", gpt4o: "Excellent structured outputs", gemini: "Good", llama: "Unreliable unfinetuned", impact: "Format constraints worth more on reliable models" },
];
...
const GRADES = [
  ...
  { g: "F", r: "0–34", note: "Broken — will produce poor/hallucinated output", c: "#fb7185" },
];
...
function gradeColor(g) {
  return { S: "#f59e0b", A: "#34d399", B: "#60a5fa", C: "#c084fc", D: "#fb923c", F: "#fb7185" }[g] || "#9ca3af";
}
...
export default function PromptScorer() {
  ...
  async function analyze() {
    ...
    } catch (e) { setError("Scoring failed — check browser console."); }
    setLoading(false);
  }
  ...
  const mono = (sz = 10, c = "#4b5563") => ({ fontFamily: "monospace", fontSize: sz, color: c, letterSpacing: "0.07em" });
  const card = { background: "#080a12", border: "1px solid #1a1d2e", borderRadius: 10 };
  const lbl = { fontFamily: "monospace", fontSize: 10, color: "#4b5563", letterSpacing: "0.1em", marginBottom: 8 };

  return (
    <div style={{ minHeight: "100vh", background: "#0d0f18", color: "#e2e8f0", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 22px", height:50, borderBottom:"1px solid #1a1d2e", background:"#080a12", position:"sticky", top:0, zIndex:50 }}>
        <div style={{ display:"flex", alignItems:"center", gap:9 }}>
          ...
          <span style={mono(13,"#e2e8f0")}>PROMPT EFFICIENCY ANALYZER</span>
          <span style={{ padding:"1px 7px", border:"1px solid #1e2030", borderRadius:4, ...mono(9) }}>PES · v1.0</span>
        </div>
        <div style={{ display:"flex", gap:14, ...mono(9) }}>
          {["5 DIMENSIONS","100 PT SCALE","CROSS-LLM ANALYSIS"].map(t=><span key={t}>{t}</span>)}
        </div>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"1fr 306px", minHeight:"calc(100vh - 50px)" }}>

        {/* LEFT */}
        <div style={{ padding:"20px 22px", borderRight:"1px solid #1a1d2e", display:"flex", flexDirection:"column", gap:16 }}>

          {/* Input */}
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
              <span style={mono(10,"#4b5563")}>INPUT PROMPT</span>
              <span style={mono(10,"#374151")}>{wc}w · {prompt.length}c</span>
            </div>
                ...
                <div style={{ display:"flex", gap:6 }}>
                  {EXAMPLES.map(ex=>(
                    <button key={ex.label} className="eb" onClick={()=>setPrompt(ex.text)}
                      style={{ padding:"3px 9px", borderRadius:4, border:"1px solid #1e2030", background:"transparent", color:"#4b5563", ...mono(10), cursor:"pointer", transition:"all 0.15s" }}>
                      {ex.label}
                    </button>
                  ))}
                </div>
                <button onClick={analyze} disabled={loading||!prompt.trim()}
                  style={{ padding:"7px 18px", borderRadius:7, background:loading||!prompt.trim()?"#1e2030":"linear-gradient(135deg,#f59e0b,#ef4444)", border:"none", color:loading||!prompt.trim()?"#374151":"#080a12", ...mono(11,"inherit"), fontWeight:700, cursor:loading||!prompt.trim()?"not-allowed":"pointer", letterSpacing:"0.08em", display:"flex", alignItems:"center", gap:6 }}>
                  {loading?<><span style={{ display:"inline-block", animation:"spin 1s linear infinite" }}>⌬</span>SCORING…</>:"ANALYZE ⌬"}
                </button>
              </div>
            </div>
            <div style={{ marginTop:5, ...mono(10,"#374151") }}>Ctrl+Enter to analyze</div>
          </div>
          ...
          {result&&(
            <div style={{ animation:"fadein 0.4s ease" }}>
              <div style={{ display:"flex", borderBottom:"1px solid #1a1d2e", marginBottom:16 }}>
                {["analysis","improved","variance"].map(t=>(
                  <button key={t} className="tb" onClick={()=>setTab(t)}
                    style={{ padding:"7px 16px", background:"transparent", border:"none", borderBottom:tab===t?`2px solid ${col}`:"2px solid transparent", color:tab===t?col:"#4b5563", ...mono(11), cursor:"pointer", textTransform:"uppercase", marginBottom:-1, letterSpacing:"0.05em", transition:"all 0.15s" }}>
              ...
              {tab==="analysis"&&(
                <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                  <div style={{ ...card, overflow:"hidden" }}>
                    <div style={{ padding:"9px 14px", borderBottom:"1px solid #1a1d2e", ...mono(10), letterSpacing:"0.1em" }}>DIMENSION BREAKDOWN</div>
                    {DIMENSIONS.map((d,i)=>{
                        ...
                        <div key={d.id} className="hr" style={{ padding:"11px 14px", borderBottom:i<4?"1px solid #0d0f18":"none", transition:"background 0.15s" }}>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
                            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                              <span style={{ ...mono(10,d.color), fontWeight:700, minWidth:28 }}>{d.abbr}</span>
                              <span style={{ fontSize:12.5, color:"#9ca3af" }}>{d.label}</span>
                            </div>
                            <span style={{ ...mono(13,d.color), fontWeight:700 }}>{val}<span style={{ color:"#374151", fontSize:10 }}>/{d.max}</span></span>
                          </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                    ...
                    <div style={{ ...card, padding:"13px 14px" }}>
                      <div style={lbl}>TOKEN ESTIMATE</div>
                      {[
                        ...
                        <div key={r.l} style={{ display:"flex", justifyContent:"space-between", marginBottom:9 }}>
                          ...
                          <span style={{ ...mono(12,r.c), fontWeight:700 }}>{r.v}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {tab==="improved"&&(
                <div style={{ display:"flex", flexDirection:"column", gap:12, animation:"fadein 0.3s ease" }}>
                  <div style={{ ...card, padding:15 }}>
                    <div style={lbl}>OPTIMIZED PROMPT</div>
                    ...
                    <button onClick={copyImproved} style={{ marginTop:9, padding:"5px 12px", border:"1px solid #1e2030", borderRadius:5, background:"transparent", color:copied?"#34d399":"#6b7280", ...mono(10), cursor:"pointer", transition:"all 0.15s" }}>
                      {copied?"✓ COPIED":"COPY PROMPT"}
                    </button>
                  </div>
                  <div style={{ ...card, padding:15 }}>
                    <div style={lbl}>WHY THESE CHANGES</div>
              ...
              {tab==="variance"&&(
                <div style={{ display:"flex", flexDirection:"column", gap:12, animation:"fadein 0.3s ease" }}>
                  <div style={{ ...card, padding:15 }}>
                    <div style={lbl}>VARIANCE FOR THIS PROMPT</div>
                    <div style={{ fontSize:13, color:"#9ca3af", lineHeight:1.65 }}>{result.llmVarianceNote}</div>
                  </div>
                  <div style={{ ...card, overflow:"hidden" }}>
                    <div style={{ padding:"9px 14px", borderBottom:"1px solid #1a1d2e", ...mono(10), letterSpacing:"0.1em" }}>CROSS-MODEL SCORING FACTORS</div>
                        ...
                        <thead>
                          <tr style={{ borderBottom:"1px solid #1a1d2e", background:"#060810" }}>
                            {["Dimension","Claude","GPT-4o","Gemini","LLaMA","Score Impact"].map(h=>(
                              <th key={h} style={{ padding:"7px 11px", textAlign:"left", ...mono(9), fontWeight:600 }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {LLM_FACTORS.map((row,i)=>(
                            <tr key={i} className="hr" style={{ borderBottom:"1px solid #0d0f18", transition:"background 0.15s" }}>
                              <td style={{ padding:"8px 11px", color:"#f59e0b", ...mono(11), fontWeight:700 }}>{row.dim}</td>
                  ...
                  <div style={{ background:"rgba(245,158,11,0.05)", border:"1px solid rgba(245,158,11,0.2)", borderRadius:10, padding:"13px 15px" }}>
                    <div style={{ ...mono(10,"#f59e0b"), marginBottom:7, letterSpacing:"0.08em" }}>ENTERPRISE IMPLICATION</div>
                    <div style={{ fontSize:12.5, color:"#9ca3af", lineHeight:1.65 }}>
                      ...
                      Tag each prompt in your library with its target model family and re-evaluate scores on every model upgrade.
                      Maintain model-specific variants of critical system prompts rather than a single universal version.
                    </div>
                  </div>
                </div>
              )}
          ...
          {!result&&!loading&&(
            <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:10, padding:32, textAlign:"center" }}>
              ...
              <div style={{ ...mono(11,"#2d3044"), letterSpacing:"0.1em" }}>PASTE A PROMPT AND CLICK ANALYZE</div>
              <div style={{ fontSize:12, color:"#2d3044", maxWidth:340, lineHeight:1.6 }}>Scored across 5 dimensions: Clarity · Context · Specificity · Token Efficiency · Model Alignment</div>
            </div>
          )}
        </div>

        {/* RIGHT */}
        <div style={{ background:"#080a12", display:"flex", flexDirection:"column" }}>

          {/* Score */}
          <div style={{ padding:"18px 18px 14px", borderBottom:"1px solid #1a1d2e" }}>
            <div style={lbl}>EFFICIENCY SCORE</div>
            {result?(
              <div style={{ animation:"fadein 0.5s ease" }}>
                <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:14 }}>
                  <ScoreRing total={result.total} grade={result.grade} color={col} />
                  <div>
                    <div style={{ ...mono(20,col), fontWeight:800, lineHeight:1 }}>{result.grade}</div>
                    <div style={{ ...mono(12,"#6b7280"), marginTop:4 }}>{result.gradeLabel}</div>
                    <div style={{ ...mono(11,"#374151"), marginTop:8 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
                  {DIMENSIONS.map(d=>(
                    <div key={d.id} style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <span style={{ ...mono(9,d.color), fontWeight:700, minWidth:26 }}>{d.abbr}</span>
                      ...
                      <span style={{ ...mono(10,d.color), fontWeight:600, minWidth:18, textAlign:"right" }}>{result.scores[d.id]}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display:"flex", justifyContent:"center", alignItems:"center", height:100 }}>
                <span style={{ ...mono(10,"#1e2030"), letterSpacing:"0.1em" }}>{loading?"SCANNING…":"— AWAITING INPUT —"}</span>
              </div>
            )}
          </div>
          ...
          <div style={{ padding:"14px 18px", borderBottom:"1px solid #1a1d2e" }}>
            <div style={lbl}>GRADE REFERENCE</div>
            {GRADES.map(row=>(
              <div key={row.g} style={{ display:"flex", alignItems:"flex-start", gap:8, marginBottom:5, padding:"4px 7px", borderRadius:5, background:result?.grade===row.g?`${row.c}10`:"transparent", border:result?.grade===row.g?`1px solid ${row.c}25`:"1px solid transparent", transition:"all 0.2s" }}>
                <span style={{ ...mono(11,row.c), fontWeight:800, minWidth:14 }}>{row.g}</span>
                <span style={{ ...mono(10,"#374151"), minWidth:46, marginTop:1 }}>{row.r}</span>
          ...
          <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
            <div style={{ padding:"10px 18px 7px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <span style={{ ...mono(10), letterSpacing:"0.1em" }}>HISTORY</span>
              {history.length>0&&<span onClick={()=>setHistory([])} style={{ ...mono(9,"#374151"), cursor:"pointer" }}>CLEAR</span>}
            </div>
            <div style={{ flex:1, overflowY:"auto", padding:"0 10px 12px" }}>
              {history.length===0?(
                <div style={{ ...mono(10,"#1e2030"), textAlign:"center", paddingTop:16, letterSpacing:"0.1em" }}>— NO HISTORY —</div>
              ):(
                history.map(item=>{
                  ...
                  return(
                    <div key={item.id} onClick={()=>{ setPrompt(item.fullPrompt); setResult(item.result); setTab("analysis"); }}
                      ...
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                        <span style={{ ...mono(11,ic), fontWeight:700 }}>{item.result.grade} — {item.result.total}/100</span>
                        <span style={mono(10,"#374151")}>{item.time}</span>
                      </div>
